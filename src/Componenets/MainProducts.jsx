  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Repeat the original 5 products 4 times to get 20 cards
        const repeated = Array.from({ length: 4 }, () => res.data.products).flat()

        // Optionally give each product a unique fake ID
        const extended = repeated.map((p, i) => ({
          ...p,
          id: `${p.id}-${i}`, // ensures unique key
        }))
